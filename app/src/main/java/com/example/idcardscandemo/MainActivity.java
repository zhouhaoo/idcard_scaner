...
import com.example.idcardscandemo.model.Data;
...
public class MainActivity extends Activity {
			...
			case IMPORT_CODE:
				...
				try {
					...
					if(!(bytes.length>(1000*1024*5))){
						...
						Toast.makeText(MainActivity.this, "图片太大！！！", Toast.LENGTH_LONG).show();
					}
	...
	private void setResult(String result) {
		...
		Log.e("qwqw", result);
		if (data != null) {
			Data.DataBean.ItemBean item = data.data.item;
			String s="姓名:"+item.name +
					"       身份证号:"+item.cardno+
					"       性别:"+item.sex+
					"       民族:"+item.folk+
					"       生日:"+item.birthday+
